...
public class Tas extends Plateau {
    ...
    /**
     * Retourne lenombre d'allumettes initiales
     */
    ...
    public boolean verifierReponse(int n, int m){
        if(n==0){
            return false;
        }
        if (m<1||m>this.nbTas){
        ...
        if((lesTas[m-1]<n)||(n>this.nbMaxAlu)){
            return false;
        }
        return true;
    }

    public void reponse(int n, int m){
        if (!verifierReponse(n,m)){
            return;
        }
        this.lesTas[n-1] -= m;
    }
    public void setNbMaxAlu(int inputMaxAlu) throws CoupInvalideException{

            this.nbMaxAlu = inputMaxAlu;
            if (this.nbMaxAlu == 0) {
                this.nbMaxAlu = 1000000;
